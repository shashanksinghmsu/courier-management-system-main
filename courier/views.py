...
def create_parcle(request):
    ...
    if request.method == 'POST':
        ...
        courier = Courier(customer=request.user, recipient_name=recipient_name,
                          recipient_email=recipient_email, title=title, description=parcle_desc, weight=weight)
        courier.save()
        ...
        pickup_faculty = AvailableDestination.objects.filter(
            pincode=pickup_obj.pincode).first().faculty
        deliver_faculty = AvailableDestination.objects.filter(
            pincode=deliver_obj.pincode).first().faculty

        # setting pickup and delivery date
        pickup_date, delivery_date = courier_date(
            pickup_faculty, deliver_faculty)
        ...
        status = CourierDeliveryStatus(courier=courier, delivery_boy=default_delivery_boy,
                                       pickup_faculty=pickup_faculty, deliver_faculty=deliver_faculty, delivered_date=delivery_date)
        status.save()
        ...
        return render(request, 'courier/courier-confirm.html', params)

        # return redirect('courier_detail', courier_id=courier.id)
        # return redirect('conform_parcle', courier_id=courier.id)
...
def conform_parcle(request, courier_id):
    ...
    # return render(request, 'courier/courier-confirm.html')
    return redirect('home')
...
def courier(request, courier_id):
    ...
    return render(request, 'courier/courier-details.html', params)
